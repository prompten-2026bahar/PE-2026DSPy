...
LLM tabanlı hassasiyet/duyarlılık (precision/recall) aracılığıyla bir tahmin (prediction) ile gerçek yanıt (ground truth) arasındaki anlamsal (semantic) F1 skorunu hesaplar.

**Parametreler:**

| İsim (Name) | Tip (Type) | Açıklama (Description) | Varsayılan (Default) |
| :--- | :--- | :--- | :--- |
| `threshold` | - | Optimizasyon sırasında kabul edilecek minimum F1 skoru. Varsayılan 0.66. | `0.66` |
| `decompositional` | - | Eğer `True` ise, `DecompositionalSemanticRecallPrecision` kullanır. Varsayılan `False`. | `False` |

*Kaynak kod: `.venv/lib/python3.14/site-packages/dspy/evaluate/auto_evaluation.py`*
