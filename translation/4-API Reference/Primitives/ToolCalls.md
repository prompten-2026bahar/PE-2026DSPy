...
Bir sözlük (dictionary) listesini bir `ToolCalls` örneğine (instance) dönüştürür.

**Parametreler:**

| İsim (Name) | Tip (Type) | Açıklama (Description) | Varsayılan (Default) |
| :--- | :--- | :--- | :--- |
| `dict_list` | - | Her bir sözlüğün 'name' (isim) ve 'args' (argümanlar) anahtarlarına sahip olması gereken sözlükler listesi. | **Gerekli (required)** |

**Dönüş Değerleri (Returns):**

| Tip (Type) | Açıklama (Description) |
| :--- | :--- |
| `ToolCalls` | Bir `ToolCalls` örneği. |
