...
**Kullanılan Yapılar (Bases):** `Module`, `Parameter`

Girdileri bir dil modeli kullanarak çıktılara eşleyen (maps) temel DSPy modülüdür.
...
| :--- | :--- | :--- | :--- |
| `signature` | `str \| type[Signature]` | Görevi açıklayan girdi/çıktı imzası. | **Gerekli (required)** |
| `callbacks` | `list[BaseCallback] \| None` | Enstrümantasyon (instrumentation) için isteğe bağlı geri çağırma (callback) listesi. | `None` |
| `**config` | - | Temeldeki (underlying) dil modeline iletilen varsayılan anahtar kelime argümanları. Bu değerler, modülü çağırırken bir `config` sözlüğü iletilerek tek bir çağrı için geçersiz kılınabilir (overridden). Örneğin:<br><br>`predict = dspy.Predict("q -> a", rollout_id=1, temperature=1.0)`<br>`predict(q="What is 1 + 52?", config={"rollout_id": 2, "temperature": 1.0})` | `{}` |

*Kaynak kod: `.venv/lib/python3.14/site-packages/dspy/predict/predict.py`*
...
Bir `Predict` nesnesinin kaydedilmiş durumunu (state) yükler.

**Parametreler:**

| İsim (Name) | Tip (Type) | Açıklama (Description) | Varsayılan (Default) |
| :--- | :--- | :--- | :--- |
| `state` | `dict` | Bir `Predict` nesnesinin kaydedilmiş durumu. | **Gerekli (required)** |
| `allow_unsafe_lm_state` | `bool` | `True` ise, serileştirilmiş LM durumundan `api_base`, `base_url` ve `model_list` değerlerini korur. Yalnızca güvenilir dosyaları yüklerken etkinleştirin. | `False` |

**Dönüş Değerleri (Returns):**

| Tip (Type) | Açıklama (Description) |
| :--- | :--- |
| `Predict` | Metot zincirlemesine (method chaining) izin vermek için `Self` (kendisini) döndürür. |
