...
**Kullanılan Yapılar (Bases):** `BaseModule`

Tüm DSPy modülleri (programları) için temel (base) sınıftır.

Bir Modül, tahmincileri (predictors), alt modülleri (sub-modules) ve özel mantığı (custom logic) içerebilen DSPy programları için bir yapı taşıdır. Modüller karmaşık boru hatları (pipelines) oluşturmak üzere bir araya getirilebilir ve DSPy'ın teleprompter'ları kullanılarak optimize edilebilirler.



Tüm DSPy programları bu sınıftan miras almalı (inherit) ve programın mantığını tanımlayan bir `forward` metodu uygulamalıdır.

**Parametreler:**

| İsim (Name) | Tip (Type) | Açıklama (Description) | Varsayılan (Default) |
| :--- | :--- | :--- | :--- |
| `callbacks` | - | Enstrümantasyon (instrumentation) ve izleme (monitoring) için isteğe bağlı geri çağırma (callback) işleyicileri listesi. | `None` |

**Öznitelikler (Attributes):**

| İsim (Name) | Tip (Type) | Açıklama (Description) |
| :--- | :--- | :--- |
| `callbacks` | - | Kayıtlı geri çağırma (callback) işleyicilerinin listesi. |
| `history` | - | Bu modül için LM (Dil Modeli) çağrı geçmişinin listesi. |
