...
Bir modülü farklı `rollout_id` değerleriyle `temperature=1.0` ayarında `N` defaya kadar çalıştırarak rafine eder (iyileştirir) ve en iyi tahmini (prediction) döndürür.

Bu modül, sağlanan modülü değişen rollout tanımlayıcılarıyla (identifiers) birden fazla kez çalıştırır ve belirtilen eşik değerini (threshold) aşan ilk tahmini veya en yüksek ödüle (reward) sahip olanı seçer. Hiçbir tahmin eşik değerini karşılamazsa, gelecekteki tahminleri iyileştirmek için otomatik olarak geri bildirim (feedback) üretir.

**Parametreler:**

| İsim (Name) | Tip (Type) | Açıklama (Description) | Varsayılan (Default) |
| :--- | :--- | :--- | :--- |
| `module` | `Module` | Rafine edilecek modül. | **Gerekli (required)** |
| `N` | `int` | Modülün çalıştırılma sayısı. | **Gerekli (required)** |
| `reward_fn` | `Callable` | Ödül fonksiyonu (reward function). | **Gerekli (required)** |
| `threshold` | `float` | Ödül fonksiyonu için belirlenen eşik değeri. | **Gerekli (required)** |
| `fail_count` | `Optional[int]` | Bir hata fırlatmadan önce modülün kaç kez başarısız olabileceği. | `None` |
...
# Tek kelimelik cevapları kontrol eden bir ödül fonksiyonu tanımlayın
def one_word_answer(args, pred):
    return 1.0 if len(pred.answer.split()) == 1 else 0.0

# En fazla 3 kez deneyen rafine edilmiş bir modül oluşturun
best_of_3 = dspy.Refine(module=qa, N=3, reward_fn=one_word_answer, threshold=1.0)

# Rafine edilmiş modülü kullanın
result = best_of_3(question="What is the capital of Belgium?").answer
# Döndürdüğü sonuç: Brussels
```

*Kaynak kod: `.venv/lib/python3.14/site-packages/dspy/predict/refine.py`*
