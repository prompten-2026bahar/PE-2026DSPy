# /translation/chain_of_thought.md
...
Bir görevin çıktısını tahmin etmek için adım adım akıl yürüten (reasons step by step) bir modüldür.

**Parametreler:**

| İsim (Name) | Tip (Type) | Açıklama (Description) | Varsayılan (Default) |
| :--- | :--- | :--- | :--- |
| `signature` | `Type[Signature]` | Modülün imzası. | **Gerekli (required)** |
| `rationale_field` | `Optional[Union[OutputField, FieldInfo]]` | Akıl yürütmeyi (reasoning) içerecek alan. | `None` |
| `rationale_field_type` | `Type` | Gerekçe (rationale) alanının tipi. | `str` |
| `**config` | `dict[str, Any]` | Modül için yapılandırma (configuration). | `{}` |

*Kaynak kod: `.venv/lib/python3.14/site-packages/dspy/predict/chain_of_thought.py`*
