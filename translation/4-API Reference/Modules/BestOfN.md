...
Bir modülü farklı `rollout_id` değerleriyle `temperature=1.0` ayarında `N` defaya kadar çalıştırır ve `N` deneme içinden en iyi tahmini (prediction) veya eşik değerini (`threshold`) geçen ilk tahmini döndürür.

**Parametreler:**

| İsim (Name) | Tip (Type) | Açıklama (Description) | Varsayılan (Default) |
| :--- | :--- | :--- | :--- |
| `module` | `Module` | Çalıştırılacak modül. | **Gerekli (required)** |
| `N` | `int` | Modülün çalıştırılma sayısı. | **Gerekli (required)** |
| `reward_fn` | `Callable[[dict, Prediction], float]` | Modüle iletilen argümanları ve elde edilen tahmini alarak skaler bir ödül (reward) döndüren ödül fonksiyonu. | **Gerekli (required)** |
| `threshold` | `float` | Ödül fonksiyonu için belirlenen eşik değeri. | **Gerekli (required)** |
| `fail_count` | `Optional[int]` | Bir hata fırlatmadan önce modülün kaç kez başarısız olabileceği. Sağlanmazsa varsayılan olarak `N` değerini alır. | `None` |
...
*Kaynak kod: `.venv/lib/python3.14/site-packages/dspy/predict/best_of_n.py`*
