# DSPy ile Yapay Zeka Ajanları İnşa Edin

Bu eğitimde, DSPy ile bir yapay zeka ajanının nasıl inşa edileceği konusunda size rehberlik edeceğiz. Yapay zeka ajanları, çevrelerini otonom olarak algılayabilen, kararlar verebilen ve belirli hedeflere ulaşmak için eyleme geçebilen sistemleri ifade eder. Tek bir model isteminden farklı olarak, bir ajan genellikle karmaşık görevleri tamamlamak için arama motorları, API'ler veya bellek gibi araçları entegre ederek bir akıl yürütme, planlama ve eylem döngüsünü takip eder.

Bu eğitim, Dil Modeline (LM) araçların bir listesiyle birlikte bir görev açıklaması sağlayan ve ardından LM'nin daha fazla gözlem için araçları mı çağıracağına yoksa nihai çıktıyı mı üreteceğine karar vermesine izin veren, **Akıl Yürütme ve Eylem** (Reasoning and Acting) anlamına gelen **ReAct** adlı popüler bir yapay zeka ajanı mimarisine odaklanmaktadır.


Demo olarak, aşağıdakileri yapabilen basit bir havayolu müşteri hizmetleri ajanı inşa edelim:

* Kullanıcı adına yeni seyahat rezervasyonu yapmak.
* Uçuş değişikliği ve iptali dahil olmak üzere mevcut seyahatleri düzenlemek.
* Yönetemediği görevlerde bir müşteri destek talebi (ticket) oluşturmak.

Bunu `dspy.ReAct` modülünü kullanarak inşa edeceğiz.
...
Ajanın gerçek bir havayolu hizmet görevlisi gibi davranabilmesi için bir araç listesi hazırlamamız gerekiyor:

* `fetch_flight_info`: Belirli tarihler için uçuş bilgilerini getirir.
* `pick_flight`: Bazı kriterlere göre en iyi uçuşu seçer.
* `book_flight`: Kullanıcı adına uçuş rezervasyonu yapar.
* `fetch_itinerary`: Rezerve edilmiş bir seyahat planının bilgilerini getirir.
* `cancel_itinerary`: Rezerve edilmiş bir seyahat planını iptal eder.
* `get_user_info`: Kullanıcıların bilgilerini getirir.
* `file_ticket`: İnsan yardımı almak için bir birikmiş iş (backlog) talebi oluşturur.

## Veri Yapısını Tanımlama

Araçları tanımlamadan önce veri yapısını tanımlamamız gerekiyor. Gerçek üretimde bu bir veritabanı şeması olacaktır. Demo olarak, basitlik adına veri yapısını sadece **pydantic modelleri** olarak tanımlıyoruz.

```python
from pydantic import BaseModel

class Date(BaseModel):
    # Dil Modeli (LLM) `datetime.datetime` belirtme konusunda pek iyi olmadığı için,
    # tarihi temsil etmek üzere özel bir sınıf tanımlıyoruz.
    year: int
    month: int
    day: int
    hour: int
...
## Sahte Veri Oluşturma (Create Dummy Data)

Havayolu ajanının çalışabilmesi için bazı sahte veriler oluşturalım. Birkaç uçuş ve kullanıcı oluşturmamız, ayrıca seyahat planları (itineraries) ve müşteri destek talepleri için boş sözlükler başlatmamız gerekiyor.
...
flight_database = {
    "DA123": Flight(
        flight_id="DA123",  # DSPy Havayolu 123
        origin="SFO",
        destination="JFK",
...
Şimdi araçları tanımlayabiliriz. `dspy.ReAct` modülünün düzgün çalışması için her fonksiyonun şunlara sahip olması gerekir:

* Aracın ne yaptığını tanımlayan bir **docstring** (dokümantasyon dizesi). Eğer fonksiyon ismi kendi kendini açıklıyorsa, docstring boş bırakılabilir.
* Argümanlar için **tür ipucu** (type hint). Bu, Dil Modelinin (LM) argümanları doğru formatta oluşturabilmesi için gereklidir.
...
def fetch_flight_info(date: Date, origin: str, destination: str):
    """Verilen tarihte kalkış noktasından varış noktasına olan uçuş bilgilerini getirir"""
    flights = []
    ...
    if len(flights) == 0:
        raise ValueError("Eşleşen uçuş bulunamadı!")
    return flights


def fetch_itinerary(confirmation_number: str):
    """Veritabanından rezerve edilmiş bir seyahat planı bilgisini getirir"""
    return itinerary_database.get(confirmation_number)


def pick_flight(flights: list[Flight]):
    """Kullanıcıların isteğine en uygun uçuşu seçer. En kısa olanı, eşitlik durumunda ise en ucuz olanı seçeriz."""
...
def _generate_id(length=8):
    # Rastgele kimlik (ID) oluşturmak için yardımcı fonksiyon
    chars = string.ascii_lowercase + string.digits
    return "".join(random.choices(chars, k=length))


def book_flight(flight: Flight, user_profile: UserProfile):
    """Kullanıcı adına bir uçuş rezervasyonu yapar."""
...
def cancel_itinerary(confirmation_number: str, user_profile: UserProfile):
    """Kullanıcı adına bir seyahat planını iptal eder."""
...
def get_user_info(name: str):
    """Verilen isme göre veritabanından kullanıcı profilini getirir."""
    return user_database.get(name)


def file_ticket(user_request: str, user_profile: UserProfile):
    """Eğer bu ajanın yönetemeyeceği bir durumsa, bir müşteri destek talebi oluşturur."""
...
Artık `dspy.ReAct` aracılığıyla ReAct ajanını oluşturabiliriz. Görevi, ajanın giriş ve çıkışlarını tanımlamak için `dspy.ReAct`'e bir imza (signature) sağlamamız ve erişebileceği araçları ona bildirmemiz gerekiyor.

```python
import dspy

class DSPyAirlineCustomerService(dspy.Signature):
    """Siz, kullanıcıların uçuş rezervasyonu yapmasına ve yönetmesine yardımcı olan bir havayolu müşteri hizmetleri ajanısınız.

    Kullanıcı isteğini yerine getirmek için size bir araç listesi verilir ve kullanıcıların isteğini
    karşılamak için doğru aracı kullanmaya karar vermelisiniz."""

    user_request: str = dspy.InputField()
    process_result: str = dspy.OutputField(
        desc=(
                "İşlem sonucunu ve kullanıcıların ihtiyaç duyduğu bilgileri (örneğin, yeni bir uçuş "
                "rezerve edildiğinde verilen onay numarası (confirmation_number)) özetleyen mesaj."
            )
        )

# ReAct ajanının tanımlanması
agent = dspy.ReAct(
...
Ajanla etkileşime girmek için isteğinizi `user_request` aracılığıyla iletmeniz yeterlidir; ajan işini yapmaya başlayacaktır.

Bir dil modeli seçin ve API anahtarlarını ayarlayın. Burada `gpt-4o-mini` kullanıyoruz, ancak diğer modellerle değiştirebilirsiniz. Dil modelinin nasıl yapılandırılacağı hakkında bilgi için lütfen [bu kılavuza](https://dspy.ai/learn/programming/models/) bakın.
...
```
Rezerve edilen seyahat planını veritabanında görebiliriz.
...
Sonuç, kullanıcı tarafından talep edilen `process_result` alanını ve cevabın arkasındaki mantığı taşıyan bir `reasoning` (akıl yürütme) alanını içerir. Buna ek olarak, aşağıdakileri barındıran bir `trajectory` (yörünge) alanına sahiptir:

* Her adımdaki **akıl yürütme (thought)**
* Her adımda Dil Modeli (LM) tarafından seçilen **araçlar (tools)**
* Her adımda LM tarafından belirlenen **araç çağırma argümanları**
* Her adımdaki **araç yürütme sonuçları**



Arka planda `dspy.ReAct`, görev açıklamasıyla birlikte araç çağırma bilgilerini biriktiren ve `max_iters` değerine ulaşana veya LM işlemi sonlandırmaya karar verene kadar bunları Dil Modeline gönderen bir döngü yürütür. Süreci daha iyi yorumlamak için, her adımda neler olup bittiğini görmek üzere `dspy.inspect_history()` komutunu kullanalım.
...
Her bir Dil Modeli (LM) çağrısında, kullanıcı mesajının görev açıklamasıyla birlikte önceki araç çağırma bilgilerini de içerdiğini görebiliyoruz.

Şimdi farklı bir görev deneyelim.
...
Eğitimi tamamladığınız için tebrikler! Bu eğitimde DSPy ile bir müşteri hizmetleri ajanının nasıl inşa edileceğini gördük. Temel çıkarımlar şunlardır:

* Araçları Python fonksiyonları olarak tanımlayın ve **docstring (dokümantasyon dizesi) ile tür ipuçları (type hints)** ekleyin.
* Görevi tanımlamak için araçları bir imza (signature) ile birlikte `dspy.ReAct` modülüne sağlayın.
* `dspy.ReAct` modülünü imzada tanımlanan giriş alanlarıyla çağırın; bu, arka planda akıl yürütme ve eylem döngüsünü başlatacaktır.